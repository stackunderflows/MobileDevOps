...
namespace MyWeather.UITest
{
    public abstract class BasePage
    {
        ...
        protected Func<string, Query> Tab;

        void InitializeCommonQueries()
        {
            if (OnAndroid)
            {
                Tab = (marked) => x => x.Id("sliding_tabs").Descendant().Marked(marked);
            }

            if (OniOS)
            {
                Tab = (marked) => x => x.Class("UITabBarButton").Marked(marked);
            }
        }

        public void GoToTab(string marked)
        {
            app.Tap(Tab(marked));
            app.Screenshot($"Selected tab: {marked}");
        }
    }
}